...
while true; do
    ...
    case $choice in
        1)
            animation_running &
            animation_pid=$!
            network_test_output=$(speedtest-cli --simple)
            kill $animation_pid
            printf "\n"
            ping_value=$(echo "$network_test_output" | grep "Ping" | awk '{print $2, $3}')
            download_value=$(echo "$network_test_output" | grep "Download" | awk '{print $2, $3}')
            upload_value=$(echo "$network_test_output" | grep "Upload" | awk '{print $2, $3}')
            print_at_position 25 "Ping: $ping_value"
            printf "\n"
            print_at_position 25 "Download: $download_value"
            printf "\n"
            print_at_position 25 "Upload: $upload_value"
            printf "\n"
